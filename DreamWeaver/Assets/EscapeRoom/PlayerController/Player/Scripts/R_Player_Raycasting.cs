...
public class R_Player_Raycasting : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            // 마우스 왼쪽 버튼이 눌렸다면
            // 부딪힌 오브젝트(hit)의 위치가 사용자 지정 변수 거리보다 작으면(지정된 사정 거리 내에 있다면)
            if (Input.GetMouseButtonDown(0) && hit.distance < rayDistance)
            {
                // 충돌한 오브젝트 태그가 Door 이고,
                ...
                // 충돌한 오브젝트 태그가 PasswordDoor이면, KeyPadController 함수 실행
                if (hit.collider.tag.Equals("PasswordDoor"))
                {
                    GameManager.instance.Keypad.gameObject.SetActive(true);
                }
