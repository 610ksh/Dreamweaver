...
{
    public float fadeInTime; // Fade애니메이션 플레이시간
    public float fadeOutTime; // Fade애니메이션 플레이시간
    ...
    IEnumerator PlayFadeEffect(Effect fadeEffect)
    {
        ...
        if (fadeEffect == Effect.FadeIn)
        {
            ...
            while (color.a > 0f)
            {
                time += Time.deltaTime / fadeInTime; // 작동시간 조절
        ...
        else if (fadeEffect == Effect.FadeOut)
        {
            ...
            while (color.a < 1f)
            {
                time += Time.deltaTime / fadeOutTime; // 작동시간 조절
                // 알파값 선형 보간법 1에서 0으로 점점 선형적으로 변함.
                color.a = Mathf.Lerp(startAlpha, endAlpha, time);
                // 바뀐 알파값을 현재 화면에 적용
                fadeImage.color = color;
